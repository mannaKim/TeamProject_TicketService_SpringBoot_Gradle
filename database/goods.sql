...
as
select * from
(select rownum,gseq,name,price2,image,num_inventory from tp_goods where useyn='y' order by indate desc)
where rownum<=4;
...
as
select * from
(select rownum,gseq,name,price2,image,num_inventory from tp_goods where bestyn='y' order by indate desc)
where rownum<=4;
